...
const validateParentCategory = (req, parent) => {
  if (!parent) {
...
export const createCategory = async (req, res, next) => {
  try {
    ...
    if (parent_id !== undefined && parent_id !== null && parent_id !== '') {
      ...
      const parentValidationError = validateParentCategory(req, parent);
    ...
    const transformRow = async (row, rowNum) => {
      ...
      if (row.parent_id && String(row.parent_id).trim() !== '') {
        ...
        if (uuidRegex.test(parentValue)) {
          parent = await Category.findByPk(parentValue);
        } else {
        ...
        if (!parent) {
          throw new Error(`Parent category "${parentValue}" not found`);
        }

        const validation = validateParentCategory(req, parent);
        if (validation) {
